...
regxvals xs @ (x : _) ("noop" : _)    = x : xs
regxvals xs @ (x : _) ("addx" : n :_) = (x + (toInt n)) : x : xs

solve2210_2 =
  flip trace 0 . concat . map (++"\n")
  . map ( map (\(rx, np) -> if ((<=) (abs (rx - np)) (1)) then '#' else ' ') )
  . map ( zip [0..] )
  . chunksOf 40
  . reverse . foldl regxvals [1] . map words . lines
